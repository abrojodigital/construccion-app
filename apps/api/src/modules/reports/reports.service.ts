...
export class ReportsService {
  async getDashboard(organizationId: string, query: { period?: string; projectId?: string }) {
    const { period = 'month', projectId } = query;
  ...
  async getProjectCosts(projectId: string, organizationId: string) {
    // Get project details
  ...
  async getProjectProgress(projectId: string, organizationId: string) {
    const project = await prisma.project.findFirst({
      where: { id: projectId, organizationId },
      include: {
        stages: {
  ...
  async getExpenses(
    ...
    query: { startDate?: string; endDate?: string; groupBy?: 'category' | 'project' | 'supplier' }
  ) {
  ...
  async getProjectBudgetControl(projectId: string, organizationId: string) {
    const project = await prisma.project.findFirst({
  ...
  async exportPdf(organizationId: string, query: { period?: string; projectId?: string }) {
    const { period = 'month', projectId } = query;
...
<body>
  ...
  <div style="margin-top: 40px; text-align: center; color: #9ca3af; font-size: 12px;">
    ...
    return { html, filename: `reporte-${new Date().toISOString().split('T')[0]}.html` };
  }

  async exportExcel(organizationId: string, query: { period?: string; projectId?: string }) {
    const { period = 'month', projectId } = query;
    ...
    return { csv: bom + csvContent, filename: `reporte-${new Date().toISOString().split('T')[0]}.csv` };
  }
}
